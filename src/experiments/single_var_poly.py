...
from src.synthetic_datasets.test_datasets import Polynomial
...
def run_experiment(seed=777, n=3):
  ...
  root = Polynomial(input_range=(-4,4), step=0.001, noise=0.5)

  trainset, testset = torch.utils.data.random_split(root, (7200, 800))
  ...
  for run in range(n):
    for rate in [ 0, 1/8, 1/4, 1/2]:
      os.makedirs(f'single-var-poly/rate{rate:.3f}', exist_ok=True)
      ...
      torch.optim.lr_scheduler.OneCycleLR(optimizer, 0.4, epochs=100, steps_per_epoch=6, pct_start=0.1, div_factor=500, final_div_factor=300, three_phase=True)
      # torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 1365, 2, 0.0001)
      ...
      net.fit(trainloader, testloader, optimizer, criterion, epochs=20, loader_transform=lambda x: (x[0].unsqueeze(-1), x[1].unsqueeze(-1)), grad_clipper=clipping_strategy, save_options=(5, f'./single-var-poly/rate{rate:.3f}/single-var-poly-rr-{rate:.3f}-run-{run}-'))