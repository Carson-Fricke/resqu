...
def run_experiment(seed=777, n=5):
  ...
  for rate in [0, 1/64, 3/32, 3/16, 1/2]:
    for run in range(n):
      ...
      torch.optim.lr_scheduler.OneCycleLR(optimizer, 0.1, epochs=300, steps_per_epoch=550, pct_start=0.1, div_factor=25, final_div_factor=300, three_phase=True)
      print('Parameter Count: ', sum(p.numel() for p in net.parameters()))
      net.fit(trainloader, testloader, optimizer, criterion, epochs=300, save_options=(None, f'resnet20-CIFAR10-rr-{rate}-run-{run}'))
